...
# -*- coding: utf-8 -*-
"""
Created on Tue May 26 18:48:42 2020
...
# %%Case 6 Drop randomly biased relab
drop_list = [0.99, 0.99, 0.8]
values = [0,1,2]
dp = zeros*0.99 + ones*0.99 + twos*0.8
states6 = dont_drop(values = values, states = states.copy(),
                    drop_perc = dp, drop_list = None)
...
inputs_fit =  {'data': data, 'states': states6, 'dates' : dates, 'save_name': save_name, 
               'states_off': states_off, 'label_mat':label_mat, 'A': A_mat, 'pi': pi}

start = time.time()

mhmm6 = MHMM(**inputs_class)
mhmm6 = mhmm6.fit(**inputs_fit)
...
A_6 = hmm_6.A.flatten()
means_6 = hmm_6.means.flatten()
cov_6 = hmm_6.cov.flatten()
pi_6 = hmm_6.pi.flatten()


A_err6 = np.sum(np.abs(A_6-A_fl))
means_err6 = np.sum(np.abs(means_6-mean_fl))
cov_err6 = np.sum(np.abs(cov_6-std_fl))
pi_err6 = np.sum(np.abs(pi_6-pi_fl))


errors.append([A_err6, means_err6, cov_err6, pi_err6])

# %% save errors
np.save('tables/errors_mat.npy', errors)
