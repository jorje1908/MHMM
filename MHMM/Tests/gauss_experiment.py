...
data, states = gauss_seq(T = T, N = N, A = A, mean = mean, std = std)
...
labels_mat = None
mhmm = MHMM(n_HMMS = n_HMMS, n_states = 2, n_Comp = n_Comp, EM_iter = EM_iter, tol = 10**(-7))
mhmm = mhmm.fit( data = data, states = states1, dates = None, save_name = None, states_off = 0,
                label_mat = labels_mat )
#get the hmm
hmm = mhmm.HMMS[0]
params6 = hmm.get_params()
cov = params6['cov'].reshape(-1, mean.shape[1], mean.shape[1])
